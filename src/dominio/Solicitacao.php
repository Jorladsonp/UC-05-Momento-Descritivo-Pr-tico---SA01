...
class Solicitacao
{
    ...
    public function __construct(
        ...
        RegraPrioridade $regraPrioridade
    ) {
        ...
        $this->prioridade   = $regraPrioridade->definirPrioridade($this);
    }
    ...
    public function adicionarObservacao(string $obs): void
    {
        $this->garantirNaoFinalizada();
        if (trim($obs) === '') throw new InvalidArgumentException("Observação não pode ser vazia.");
        $this->observacao = trim($obs);
    }

    public function recalcularPrioridade(RegraPrioridade $regra): void
    {
        $this->garantirNaoFinalizada();
        $this->prioridade = $regra->definirPrioridade($this);
    }
    ...
    {
        if (trim($valor) === '') {
            throw new InvalidArgumentException("{$campo} é obrigatório.");
        }
    }
