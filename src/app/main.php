...
require_once __DIR__ . '/../dominio/RegraPrioridade.php';
require_once __DIR__ . '/../dominio/RegraPrioridadeGeral.php';
require_once __DIR__ . '/../dominio/RegraPrioridadeManutencao.php';
...
$regraGeral      = new RegraPrioridadeGeral();
$regraManutencao = new RegraPrioridadeManutencao();
...
// ---------------------------------------------------------------
while (true) {
    cabecalho('SISTEMA DE SOLICITAÇÕES INTERNAS');
    ...
    echo " 11. Demonstração de polimorfismo" . PHP_EOL;
    echo "  0. Sair" . PHP_EOL;
    linha();
    ...
    switch ($opcao) {
        ...
        case '1':
            ...
            $regra = ($categoria === Categoria::MANUTENCAO) ? $regraManutencao : $regraGeral;

            try {
                $s = new Solicitacao($titulo, $descricao, $categoria, $solicitante, $regra);
                $repo->adicionar($s);
                echo PHP_EOL . "  Solicitação criada com sucesso:" . PHP_EOL;
                echo "  {$s}" . PHP_EOL;
                echo "  (Regra usada: " . $regra->descricaoRegra() . ")" . PHP_EOL;
        ...
        case '11':
            cabecalho('DEMONSTRAÇÃO DE POLIMORFISMO');
            echo "  Mesma solicitação avaliada por regras diferentes:" . PHP_EOL . PHP_EOL;

            $demo = new Solicitacao(
                'Bomba de resfriamento',
                'Equipamento parado, produção paralisa sem reparo imediato.',
                Categoria::MANUTENCAO,
                $solicitante1,
                $regraGeral
            );

            echo "  [Regra Geral]      Prioridade: {$demo->getPrioridade()->value}" . PHP_EOL;
            echo "  Desc: " . $regraGeral->descricaoRegra() . PHP_EOL . PHP_EOL;

            $demo->recalcularPrioridade($regraManutencao);
            echo "  [Regra Manutenção] Prioridade: {$demo->getPrioridade()->value}" . PHP_EOL;
            echo "  Desc: " . $regraManutencao->descricaoRegra() . PHP_EOL;
            echo PHP_EOL . "  -> Mesma chamada recalcularPrioridade(), resultados diferentes = polimorfismo." . PHP_EOL;
            break;
